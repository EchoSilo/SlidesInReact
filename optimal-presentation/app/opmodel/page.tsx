  ...
  const exportAllSlidesAsPNG = async () => {
    ...
    try {
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        zip.file(`opmodel-slide-${i + 1}-${slides[i].id}.png`, blob)
      }
      ...
      const link = document.createElement("a")
      link.download = "opmodel-slides.zip"
  ...
  const exportAllSlidesAsPPTX = async () => {
    ...
    try {
        ...
        try {
          ...
          if (currentSlideData.type === "title") {
            ...
            if (currentSlideData.content) {
              ...
              const contentArrays = [
                currentSlideData.content.problems,
                currentSlideData.content.areas,
                currentSlideData.content.components,
                currentSlideData.content.rhythms,
                currentSlideData.content.phases,
                currentSlideData.content.requirements,
                currentSlideData.content.principles,
      ...
      await pptx.writeFile({
        fileName: "Operating-Model-Framework"
      })
