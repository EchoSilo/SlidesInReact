    ...
    try {
      ...
      const slideData = slides.map((slide) => {
        ...
        if (slide.content) {
          ...
          if (slide.type === 'challenge' && slide.content.problems) {
            contentText = slide.content.problems.map((p: any) =>
              `• ${p.title}: ${p.description}`
            ).join('\n');
          } else if (slide.content.benefits) {
            contentText = slide.content.benefits.map((b: any) =>
              `• ${b.title}: ${b.description} (${b.impact})`
            ).join('\n');
          } else if (slide.content.steps) {
            contentText = slide.content.steps.map((s: any) =>
              `${s.number || s.phase || ''} ${s.title}: ${s.desc || s.description || ''}`
            ).join('\n');
          } else if (slide.content.outcomes) {
            contentText = slide.content.outcomes.map((o: any) =>
              `• ${o.title}: ${o.metric} - ${o.desc}`
            ).join('\n');
      ...
      await exportToPPTX(
        slideData,
        'Capacity-Management-Framework.pptx',
